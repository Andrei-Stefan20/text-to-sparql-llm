...
class PromptBuilder:
    ...
    def _format_entities(self, entities: List) -> str:
        ...
        for entity in entities:
            if hasattr(entity, "to_sparql_format"):
                ...
                # Dict with qid
                formatted.append(
                    f"{entity.get('text', str(entity))} (wd:{entity['qid']})"
                )
            else:
                # Plain string
    ...
    def build_user_prompt(
        ...
        if self.strategy == PromptStrategy.COT:
            ...
            return self._build_decomposition_prompt(
                question, entities, context_examples, schema_hints
            )
        else:
...
"""
    ...
    def extract_sparql_from_response(
        self, response: str, validate_syntax: bool = True
    ) -> str:
        """
    ...
    def _extract_from_code_blocks(self, response: str) -> Optional[str]:
        ...
        pattern = (
            r"```(?:sparql)?\s*((?:PREFIX|SELECT|CONSTRUCT|ASK|DESCRIBE)[\s\S]*?)```"
        )
    ...
    def _clean_sparql(self, query: str) -> str:
        ...
        query = re.sub(
            r"^(?:SPARQL|Query|Final Query|Corrected Query)[:\s]+",
            "",
            query,
            flags=re.IGNORECASE,
        )
