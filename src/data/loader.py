...
class DatasetLoader:
    ...
    def load(self) -> List[Dict[str, Any]]:
        ...
        if local_path and os.path.exists(local_path):
            ...
            try:
                ...
                logger.error(
                    f"Error loading local file: {e}. Falling back to standard download..."
                )
        else:
            logger.warning(f"Local file not found at '{local_path}'.")
            logger.warning(
                "Please ensure you have unpacked the QALD data in 'data/raw/'."
            )
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                sparql = (
                    row.get("query.sparql") or row.get("sparql") or row.get("query")
                )
                uid = row.get("id")
    ...
    def _load_local_json(self, path: str) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Successfully processed {len(processed_data)} items for language '{self.language}'."
        )
