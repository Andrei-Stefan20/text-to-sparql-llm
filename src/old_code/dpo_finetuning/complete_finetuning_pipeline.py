...
def inference_stage(
    ...
    logger.info(f"{len(results)} samples, {correct} correct ({100*correct/len(results):.1f}%)\n")
...
def create_dpo_dataset(
    ...
    logger.info(f"Created {len(dpo_pairs)} pairs ({100*len(dpo_pairs)/len(inference_results):.1f}% error rate)\n")
...
def evaluate_model(
    ...
    for sample in tqdm(dataset, desc="Evaluating"):
        ...
        try:
            generated = engine.generate(prompt=prompt, temperature=0.3, max_new_tokens=512)
            is_correct = QueryComparator.is_correct(generated, gold_query)
...
def run_pipeline(
    ...
    try:
        ...
        results_summary['finetuned_model'] = str(ft_model_path) if ft_model_path else None
