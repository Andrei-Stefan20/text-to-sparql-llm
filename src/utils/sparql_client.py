...
class SPARQLClient:
    ...
    def __init__(self, endpoint_url: str = "https://query.wikidata.org/sparql", user_agent: str = "TextToSparqlBot/1.0", timeout: int = 60):
    ...
    def validate_syntax_local(self, query: str) -> Dict[str, Any]:
        ...
        if not query:
            return {"valid": False, "type": "Empty Output", "detail": "No query generated"}
        
        if not isinstance(query, str):
            return {"valid": False, "type": "Type Error", "detail": f"Expected str, got {type(query).__name__}"}
    ...
    def execute_remote(self, query: str, timeout_override: Optional[int] = None) -> Tuple[Optional[List], Optional[str]]:
        """
    ...
    def calculate_f1(self, gold_results: Optional[List], gen_results: Optional[List]) -> float:
        """
