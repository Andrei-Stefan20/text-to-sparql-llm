...
class GeminiQueryGenerator:
    ...
    def __init__(self, model_id: str, max_retries: int = 2):
        ...
        self.min_request_interval = (
            5.0  # 5 seconds between requests (free tier: 15/min)
        )
    ...
    def generate_raw(
        self, prompt: str, stop: Optional[List[str]] = None, max_new_tokens: int = 1024
    ) -> str:
        """
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                if "429" in error_msg or "quota" in error_msg.lower():
                    ...
                    logger.warning(
                        f"API request failed (attempt {attempt}), retrying in {wait_time}s: {error_msg[:80]}"
                    )
                    time.sleep(wait_time)
                else:
                    logger.error(
    ...
    def process_question(q_idx_tuple):
        ...
        try:
            ...
            for attempt in range(1, MAX_RETRIES + 2):
                try:
                    raw_response = generator.generate_raw(
                        prompt, stop=["User:", "###", "Question:"]
                    )
                    ...
                    best_result = (
                        ...
                        "",
                        False,
                        {"type": "API Error", "detail": str(e)[:100]},
                        None,
                        attempt,
                    )
                    break
                    ...
                    best_result = (
                        ...
                        "",
                        False,
                        {"type": type(e).__name__, "detail": str(e)[:100]},
                        None,
                        attempt,
                    )
                    break
            ...
            gen_sparql, raw_resp, is_valid, error_info, gen_results, attempts = (
                best_result
            )
