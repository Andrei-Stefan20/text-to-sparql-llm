...
class QueryProcessor:
    ...
    def run(self, user_question: str, question_id: int = 1) -> str:
        ...
        for step_idx, step in enumerate(steps_plan):
            ...
            try:
                results, metadata = self.executor.execute_step(
                ...
                if metadata.get("status") == "failed":
                    ...
                    logger.info(
                        f"Step {step_num} completed successfully. Results: {metadata.get('result_count', 0)} items"
                    )
        ...
        final_answer = self._combine_results(
            user_question, steps_plan, execution_results
        )
    ...
    def _combine_results(
        self, question: str, steps: List[Dict], results: Dict[str, Any]
    ) -> str:
        """
        ...
        logger.info(
            f"Synthesizing results from {len(successful_results)} successful steps"
        )

        # Build synthesis prompt
        synthesis_prompt = self._build_result_prompt(
            question, steps, successful_results
        )
    ...
    def _build_result_prompt(
        self, question: str, steps: List[Dict], results: Dict
    ) -> str:
        """
        ...
        for i, step in enumerate(steps):
            ...
            results_summary.append(
                f"Step {i+1} ({step.get('description', 'N/A')[:70]}): {result_count} items found"
            )
        ...
        for i, step in enumerate(steps):
            ...
            if result_list and isinstance(result_list, list):
                ...
                if len(result_list) > 5:
                    detailed_results.append(
                        f"  ... and {len(result_list) - 5} more items"
                    )
    ...
    def _determine_aggregation_type(self, question: str, steps: List[Dict]) -> str:
        ...
        if any(
            ...
            for word in ["count", "how many", "total", "number of", "sum", "average"]
        ):
