...
class StepRunner:
    ...
    def execute_step(
        self, step_dict: Dict, context_history: Dict, current_step_idx: int = None
        ...
        if depends_on is not None:
            if depends_on < 0:
                logger.error(
                    f"Invalid step dependency: step {depends_on + 1} cannot be negative."
                )
                return None, {
                    "status": "failed",
                    "reason": "invalid_dependency",
                    "detail": f"Step number must be positive, got {depends_on + 1}",
                }
            if current_step_idx is not None and depends_on >= current_step_idx:
                logger.error(
                    f"Invalid step dependency: step {depends_on + 1} is not before current step {current_step_idx + 1}."
                )
                return None, {
                    "status": "failed",
                    "reason": "forward_dependency",
                    "detail": f"Step {depends_on + 1} not yet executed",
                }
            if depends_on not in context_history:
                logger.warning(
                    f"Step depends on step {depends_on + 1} which has not been executed yet or failed."
                )
                return None, {
                    "status": "skipped",
                    "reason": "missing_dependency",
                    "detail": f"Step {depends_on + 1} not available",
                }
        ...
        sparql_query = self._generate_sparql_for_step(
            step_description, query_type, context_str, depends_on, context_history
        )

        if not sparql_query:
            return None, {"status": "failed", "reason": "query_generation_failed"}

        results, execution_info = self._execute_with_retry(
            sparql_query, step_description
        )

        logger.info(
            f"Step result: {len(results) if results else 0} items retrieved from Wikidata."
        )
    ...
    def _generate_sparql_for_step(
        ...
        if depends_on is not None and depends_on in context_history:
            ...
            if dep_results:
                results_preview = (
                    str(dep_results)[:200]
                    if isinstance(dep_results, list)
                    else str(dep_results)
                )
                dependency_context = (
    ...
    def _get_query_type_guidance(self, query_type: str) -> str:
        ...
        return guidance.get(
            query_type, "Generate a SPARQL query matching the description."
        )

    def _execute_with_retry(
        self, query: str, step_description: str
    ) -> Tuple[Optional[List], Dict]:
        """
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                if results:
                    ...
                    logger.warning(
                        f"No results returned. Retrying with adjusted query parameters..."
                    )
