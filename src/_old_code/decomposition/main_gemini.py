...
class GeminiQueryModel:
    ...
    def __init__(self, model_id: str):
        ...
        if not api_key:
            logger.error(
                "GEMINI_API_KEY mancante! Controlla il file .env nella root del progetto."
            )
            sys.exit(1)
...
class WikidataClientGemini:
    ...
    def __init__(self):
        ...
        self.endpoint.addCustomHttpHeader(
            "User-Agent", "TextToSparqlBot/1.0 (GeminiDecomp)"
        )
...
class SPARQLEvaluator:
    ...
    def __init__(self, model_id: str, retriever: ExampleRetriever):
        ...
        self.endpoint.setReturnFormat(JSON)
        self.endpoint.addCustomHttpHeader(
            "User-Agent", "TextToSparqlBot/1.0 (GeminiEval)"
        )
        self.endpoint.setTimeout(60)
    ...
    def generate_raw(
        self, prompt: str, stop: Optional[List[str]] = None, max_new_tokens=512
    ) -> str:
        generation_config = genai.types.GenerationConfig(
            max_output_tokens=max_new_tokens, temperature=0.1, stop_sequences=stop
        )
        # Disable safety content filters to prevent false positives when processing code.
        safety_settings = {
            HarmCategory.HARM_CATEGORY_HATE_SPEECH: HarmBlockThreshold.BLOCK_NONE,
    ...
    def generate_smart(self, question: str, gold_sparql: str, max_retries=3):
        ...
        for attempt in range(1, max_retries + 2):
            ...
            if syntax_check["valid"]:
                ...
                error_info = {
                    "type": syntax_check["type"],
                    "detail": syntax_check["detail"],
                }
...
def main():
    ...
    reporter = ReportManager(
        PROJECT_ROOT, clean_model_name, run_prefix="decomposition", mode="decomposition"
    )

    orchestrator = QueryProcessor(
        llm=llm, generator=llm, retriever=kg_client, reporter=reporter
    )
    ...
    logger.info(
        f"Starting decomposition-based evaluation on {len(test_questions)} questions using Gemini..."
    )
