...
1. **Does Chain-of-Thought help?** - Hypothesis: CoT should help with complex multi-hop questions
...
6. **Tool Dependency vs. Internal Knowledge (Ablation)**
* Do models actually need the Entity Linker, or do they know common QIDs (e.g., USA = Q30)?
* Do they need Schema Retrieval, or do they know properties (e.g., P31 = instance of)?
...
We test "Pure Parametric Knowledge" (what the model knows internally) vs. "Assisted Knowledge".

### Configurations to test:

1. **Pure LLM**: No linker (`linking=none`), No schema/examples (`retrieval=none`). The model must guess QIDs (e.g., Q30 for USA) and properties.
2. **Linker Only**: Linker enabled, but no schema retrieval.
3. **Retrieval Only**: No linker, but 3-shot retrieval (provides context/examples).
...
# === GPT-4-mini ===
# 1. Pure LLM (Blind)
python main.py dataset.limit=100 model=azure_gpt4_mini linking=none retrieval=none system.experiment_name=exp7_pure_gpt4mini_100

# 2. Linker Only (No schema context)
python main.py dataset.limit=100 model=azure_gpt4_mini linking=rebel retrieval=none system.experiment_name=exp7_linker_only_gpt4mini_100

# 3. Retrieval Only (Context but no Linker)
python main.py dataset.limit=100 model=azure_gpt4_mini linking=none retrieval=3shot system.experiment_name=exp7_rag_only_gpt4mini_100


# === GPT-4 ===
# 1. Pure LLM
python main.py dataset.limit=100 model=azure_gpt4 linking=none retrieval=none system.experiment_name=exp7_pure_gpt4_100

# 2. Linker Only
python main.py dataset.limit=100 model=azure_gpt4 linking=rebel retrieval=none system.experiment_name=exp7_linker_only_gpt4_100
...
| --- | --- | --- | --- |
| **Pure LLM** (No tools) | ___/100 | ___/100 | ___/100 |
| **Linker Only** | ___/100 | ___/100 | ___/100 |
| **Retrieval Only** | ___/100 | ___/100 | ___/100 |
| *Full System (Reference)* | ___/100 | ___/100 | ___/100 |

### Analysis:

* **Hypothesis Check**: If "Pure LLM" > "Linker Only", then our Linker is broken/harmful.
* **Parametric Knowledge**: Does GPT-4 know QIDs better than GPT-4-mini? ___
...
| 4A | 0-shot (100) |  |  |
| 4C | 3-shot (100) |  |  |
| 7A | Pure LLM (Ablation) |  | No Linker, No RAG |
